            ...
            <div class="flex w-full h-4/5">
                <form action="{{route('courses.store')}}" method="post" class="w-full h-full flex flex-row-reverse">
                    ...
                    <div class="w-1/2 h-full flex flex-col items-end pr-20 relative">
                        ...
                        <div class="w-5/6 h-auto flex flex-row-reverse justify-between pt-4 mb-6">
                            <label for="all_semesters" class="font-semibold ml-5">: all semesters</label>
                            <input type="number" name="all_semesters" value="{{old('all_semesters')}}" id="all_semesters" class="w-2/5 h-8 rounded outline-0 p-2 border border-gray-400">
                            @error('all_semesters')
                            <p class="text-red-700">{{$message}}</p>
                            @enderror
                        </div>
